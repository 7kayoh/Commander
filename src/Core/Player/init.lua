	...
	if cachedData then
		...
		for _, module: ModuleScript in ipairs(Player.Extensions.Checkers) do
			...
			elseif not status then
				error(
					"Error occurred when checking player's admin with checker "
						.. module:GetFullName()
						.. "\n\n"
						.. response
				)
...
function Player.addChecker(checker: ModuleScript): boolean
	assert(
		type(require(checker).Target.Main) == "function",
		"Checker must include a function called Main inside Package.Target"
	)
...
function Player.addDataFetcher(fetcher: ModuleScript): boolean
	assert(
		type(require(fetcher).Target.Main) == "function",
		"Fetcher must include a function called Main inside Package.Target"
	)
